...
A: Data is collected in real-time from actual user visits and aggregated hourly.

**Q: What date range should I select?**  
A: 
- For trend analysis: 7+ days
- For investigating specific issues: Narrow to the day of the incident
- For comparing performance: Select equivalent periods (week over week)

**Q: Why do I see different numbers for the same time period?**  
A: Data continues to aggregate as new user sessions complete. Recent hours may show incomplete data.

---

### Error Analysis Questions

**Q: What causes "missing resources"?**  
A: Common causes include:
- Files deleted or moved without updating code references
- Third-party services being temporarily unavailable
- Network issues between user and server
- Ad blockers or privacy extensions blocking certain resources
- CDN configuration problems
- Case-sensitivity issues in file paths

**Q: Are all errors equally important?**  
A: No. Prioritize by:
1. **JavaScript errors** - Often break form functionality completely
2. **CSS errors** - Affect appearance but usually not function
3. **Image errors** - Usually cosmetic impact only

**Q: How do I fix a missing resource?**  
1. Verify the file exists at the exact URL shown
2. Check for typos or case-sensitivity issues in the path
3. Confirm third-party services are operational
4. Test in different browsers and networks
5. Check CDN/hosting configuration

**Q: Why do errors spike at certain hours?**  
A: Possible causes:
- Deployments or code changes during that hour
...
**Q: What's a good load time for my form?**  
| Time | Rating | User Impact |
|------|--------|-------------|
| Under 1 second | Excellent | Users perceive instant loading |
| 1-2 seconds | Good | Users notice but accept the wait |
| 2-3 seconds | Needs improvement | Users may start losing patience |
| Over 3 seconds | Poor | Significant risk of user abandonment |

**Q: Why do load times vary throughout the day?**  
A: Load times fluctuate based on:
- Server load (more concurrent visitors = potentially slower)
- User geography (farther from server = slower)
- User devices (older devices = slower processing)
- Network conditions (mobile data vs. WiFi)
- Time-based traffic patterns

**Q: What should I do if times are consistently slow?**  
Consider:
...
7. Upgrade server capacity

**Q: What's the difference between Mean and Median?**  
- **Median (p50)**: The "typical" experience - not affected by extremes
- **Mean (Average)**: Can be skewed by very fast or very slow outliers

If Mean is much higher than Median, you have some users with very slow experiences that need investigation.
...
A: Depends on your form type:
- Simple contact forms: 30-50% fill rate expected
- Complex application forms: 15-30% fill rate is normal
- If fill rate < 10%, investigate usability issues

**Q: Why would clicks be high but fills be low?**  
Possible causes:
- Users clicking around but confused about what to do
- Form fields not clearly visible or labeled
- Users abandoning due to complexity or requirements
- Technical issues preventing text input
